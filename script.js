...
const API = {
  ...
  async getSongs() {
    try {
      ...
      if (!response.ok) throw new Error("Could not find songs.json");
      
      const data = await response.json();
      
      // MAGIC LINE: Handles both [ ] and { "songs": [ ] } formats
      const finalSongs = data.songs ? data.songs : data; 
      
      console.log("Songs loaded successfully:", finalSongs.length);
      return finalSongs;
...
const navOptions   = document.querySelectorAll(".nav-option[data-section]");
...
function setGreeting() {
  ...
  greetingRow.innerHTML = `<h1>${greet}</h1>`;
}
...
function loadSong(index, autoPlay = true) {
  ...
  document.querySelectorAll(".card.playing, .quick-pick-card.playing").forEach(el => el.classList.remove("playing"));
...
playBtn.addEventListener("click", () => {
  ...
  if (isPlaying) {
    ...
    audio.play().catch(() => showToast("No audio file found. Add MP3s to /assets/"));
    setPlaying(true);
  }
});
  ...
  songs.forEach((song, i) => {
    ...
    div.innerHTML = `
      <div class="quick-pick-img">
        <img src="${song.cover}" alt="${song.title}" onerror="this.parentElement.innerHTML='<i class=\\'fa-solid fa-music\\'></i>'">
      </div>
      <span class="quick-pick-name">${song.title}</span>
      <button class="quick-pick-play" title="Play ${song.title}"><i class="fa-solid fa-play"></i></button>
    `;
...
async function renderSections() {
  ...
  for (let s = 0; s < 3; s++) {
    ...
    skeletonSection.style.marginBottom = "32px";
    skeletonSection.innerHTML = `
      <div class="section-header"><div class="skeleton" style="width:180px;height:24px;border-radius:4px;"></div></div>
      <div class="cards-row">${Array(5).fill(`
        <div class="card">
      ...
      `).join("")}</div>
    `;
    sectionsContainer.appendChild(skeletonSection);
  }
    ...
    section.items.forEach((item) => {
      ...
      card.innerHTML = `
        <div class="card-img-wrap">
          ...
          <button class="card-play-btn" title="Play ${item.name}"><i class="fa-solid fa-play"></i></button>
        </div>
      ...
      card.querySelector(".card-play-btn").addEventListener("click", (e) => {
        e.stopPropagation();
        // Play a random song when a playlist card is clicked
...
document.addEventListener("keydown", (e) => {
  ...
  switch (e.code) {
    ...
    case "ArrowUp":    e.preventDefault(); audio.volume = Math.min(1, audio.volume + 0.05); volumeSlider.value = audio.volume; updateVolumeUI(audio.volume); break;
    case "ArrowDown":  e.preventDefault(); audio.volume = Math.max(0, audio.volume - 0.05); volumeSlider.value = audio.volume; updateVolumeUI(audio.volume); break;
...
mainContent.addEventListener("scroll", () => {
  document.querySelector(".sticky-nav").classList.toggle("scrolled", mainContent.scrollTop > 10);
});
